# Diagram-Driven README.md Rewrite Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) into a premium, flow-driven, and highly visual guide featuring Mermaid diagrams, LaTeX risk equations, timezone mapping timelines, and harmonized parameter guides.

**Architecture:** Restructure the document into six visual-first sections. Visualizing core decision flows, timezone boundaries, and parameter mappings allows for high developer comprehension while maintaining absolute technical precision.

**Tech Stack:** GitHub Flavored Markdown, Mermaid.js, LaTeX Math, Shell scripting commands.

---

### Task 1: Repository Blueprint & Onboarding Quick Start

**Files:**
- Modify: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) (Lines 1-52)

- [ ] **Step 1: Rewrite Title, Modern Badges, and Quantitative Summary**
  Replace lines 1-13 in [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) with:
  ```markdown
...
- [ ] **Step 2: Add Codebase Directory Tree Diagram**
  Introduce a comprehensive visual ASCII folder layout right after the header:
  ```markdown
...
- [ ] **Step 3: Update 1-Minute Quick Start Guide**
  Detail the onboarding commands for Python, MT5, and Pine Script cleanly. Ensure file links are correct.
  ```markdown
...
- [ ] **Step 4: Verify formatting of onboarding guide**
  Check that markdown rendering of the header, ASCII tree, and quick start is syntactically correct.

- [ ] **Step 5: Commit**
  ```bash
  git add README.md
  git commit -m "docs: rewrite title, badges, ASCII directory blueprint, and quickstart"
  ```

---

### Task 2: Core Strategy Logic & Entry Cards

**Files:**
- Modify: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) (Lines 53-82)

- [ ] **Step 1: Add Mermaid Strategy Decision Loop Diagram**
  Create a flow chart outlining the candle evaluation loop:
  ```markdown
...
- [ ] **Step 2: Rewrite Strategy Entry/Exit Conditions Cards**
  Use beautiful side-by-side styled lists or highly structured markdown tables to outline the entry conditions:
  ```markdown
...
- [ ] **Step 3: Update High Win-Rate Preset Table**
  Keep the optimized Gold 15m settings table aligned:
  ```markdown
...
- [ ] **Step 4: Verify Mermaid flowchart rendering**
  Inspect Mermaid code blocks to ensure no syntax errors.

- [ ] **Step 5: Commit**
  ```bash
  git add README.md
  git commit -m "docs: add Mermaid decision flow diagram and entry rules"
  ```

---

### Task 3: Timezone Conversion Timeline

**Files:**
- Modify: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) (Lines 105-135)

- [ ] **Step 1: Add Mermaid Time Zone Conversion Timeline**
  Illustrate the conversion using a visual timeline:
  ```markdown
...
- [ ] **Step 2: Commit**
  ```bash
  git add README.md
  git commit -m "docs: add Mermaidtimezone Gantt conversion chart"
  ```

---

### Task 4: Advanced Capital Safeguards

**Files:**
- Modify: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) (Lines 136-163)

- [ ] **Step 1: Rewrite Math Risk Section with LaTeX equations**
  Formulate position compounding cleanly:
  ```markdown
...
- [ ] **Step 2: Update Daily Calendar Rollover and Weekend Liquidation Details**
  Explain the rollover month/year safety logic and weekend force-close mechanics:
  ```markdown
...
- [ ] **Step 3: Commit**
  ```bash
  git add README.md
  git commit -m "docs: update risk controls, daily rollover, and weekend log mitigation"
  ```

---

### Task 5: Harmonized Parameter Reference Guide

**Files:**
- Modify: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md) (Lines 164-230)

- [ ] **Step 1: Rewrite Python Backtest CLI Parameter Table**
  Add the missing CLI options: `--input-tz`, `--bb-period`, and `--bb-dev`.
  ```markdown
  ...
  | :--- | :--- | :--- | :--- |
  | `--symbol` | (Required) | `str` | Trading asset, e.g., `XAUUSD`, `EURUSD` |
  ...
  | `--rr` | `1:2` | `str` | Risk-to-Reward ratio (e.g., 2.0 or 1:2) |
  ...
  | `--input-tz` | `None` | `str` | Input timezone of start/end hours (e.g., America/New_York) |
  | `--bb-period` | `15` | `int` | Bollinger Bands period |
  | `--bb-dev` | `1.5` | `float` | Bollinger Bands standard deviation deviation |
  ```

- [ ] **Step 2: Rewrite MT5 EA Inputs Table**
  Clean up variable listing and remove duplicate `InpMagic` row.
  ```markdown
...
- [ ] **Step 3: Rewrite TradingView Pine Script Inputs Table**
  Map public display names explicitly to internal variables for easier developer edits.
  ```markdown
  ### 3. TradingView Pine Script (`BreakoutFollowTrend_Strategy.pine`)
  | UI Input Name | Internal Variable | Default | Type | Description |
  | :--- | :--- | :--- | :--- | :--- |
  | `Risk % per Trade` | `inpRiskPct` | `2.0` | `float` | Risk % per trade |
  | `Risk:Reward Ratio`| `inpRR` | `2.0` | `float` | Risk Reward Ratio |
  | `ATR Multiplier (SL)`| `inpATRMult` | `2.0` | `float` | ATR Multiplier for Stop Loss |
  | `Use Compounding Risk`| `inpCompound` | `true` | `bool` | Use Compounding Risk based on current equity |
  | `Fixed Balance` | `inpFixedBal` | `10000.0`| `float` | Fixed balance if Compounding is off |
  | `Use EMA Trend Filter`| `inpUseEMA` | `true` | `bool` | Enable EMA Filter |
  | `EMA Period` | `inpEMAPeriod` | `200` | `int` | EMA Trend Filter Period |
  | `Bollinger Bands Period`| `inpBBPeriod`| `15` | `int` | Bollinger Bands Period |
  | `Bollinger Bands Deviation`| `inpBBDev`| `1.5`| `float` | Bollinger Bands Standard Deviation |
  | `ATR Period` | `inpATRPeriod` | `14` | `int` | ATR Period for Stop Loss calculation |
  | `Use Volume Filter`| `inpUseVol` | `true` | `bool` | Enable Volume Filter |
  | `Volume MA Period`| `inpVolPeriod`| `15` | `int` | Volume MA Period |
  | `Daily Loss Limit %`| `inpDailyLoss` | `2.0` | `float` | Daily loss limit % |
  | `Max Concurrent Trades`| `inpMaxTrades` | `1` | `int` | Max concurrent open positions |
  | `Start Hour (0-23)`| `inpStartHour` | `7` | `int` | Trading start hour |
  | `End Hour (0-23)` | `inpEndHour` | `20` | `int` | Trading end hour |
  | `Weekend Close` | `inpWeekendCl` | `false` | `bool` | Enable Friday evening close |
  | `Friday Close Time`| `inpFridayTime`| `"2345"`| `string` | Friday Time to close (HHMM) |
  ```

- [ ] **Step 4: Commit**
  ```bash
  git add README.md
  git commit -m "docs: finalize harmonized parameter references in README"
  ```
...
**Files:**
- Test: [README.md](file:///c:/Users/jessa/Nextcloud/Documents/Code/breakout-follow-101/README.md)

- [ ] **Step 1: Run Markdown structural validation**
  Verify that all formatting elements, code brackets, tables, and Mermaid flow charts render correctly without syntax anomalies.
- [ ] **Step 2: Commit all design specifications and final states**
  ```bash
  git add docs/superpowers/specs/2026-05-21-diagram-driven-readme-design.md
  git commit -m "docs: add premium design specification document for README rewrite"
  ```
